...
async function refreshOrders(){
	...
	if(orders.length !== 0){
		for(var i = 0; i < orders.length; i++){
			...
			const id = order.id;
			document.getElementById('orderlist').innerHTML += `<div class="order"><div class="id">${order.id}</div><div class="date">${order.datetime.substring(0,10)}</div><div class="total">$${order.price}</div></div>`;
		}
	} else {
